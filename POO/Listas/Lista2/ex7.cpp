...
class Fracao{
    ...
    BigInt _denominador;
public:
    Fracao(BigInt numerador, BigInt denominador);
...
Fracao::Fracao(BigInt numerador = BigInt(0), BigInt denominador = BigInt(1)){
    if(numerador == 0){
        _numerador = 0;
        _denominador = 1;
    }
...
Fracao operator +(const Fracao &a, const Fracao &b){
    ...
    if(!(a._numerador == zero) && !(b._numerador == zero) && !(a._denominador == b._denominador)) return Fracao(a._numerador*b._denominador + b._numerador*a._denominador, 
    a._denominador*b._denominador);
    

    else if(a._numerador == zero) return Fracao(b._numerador * BigInt(1), b._denominador * BigInt(1));
    
    else if(b._numerador == zero) return Fracao(a._numerador * BigInt(1), a._denominador * BigInt(1));

    else return Fracao( a._numerador + b._numerador, a._denominador * BigInt(1));
...
Fracao operator *(const Fracao &a, const Fracao &b){
    if(a._numerador == b._denominador) return Fracao(b._numerador * BigInt(1), a._denominador *BigInt(1));
    if(b._numerador == a._denominador) return Fracao(a._numerador * BigInt(1), b._denominador *BigInt(1));    
    return Fracao(a._numerador * b._numerador, a._denominador * b._denominador);
}

Fracao operator /(const Fracao &a, const Fracao &b){
    return a*Fracao(b._denominador * BigInt(1), b._numerador * BigInt(1));
}

std::ostream& operator <<(std::ostream &out, const Fracao &a){
    
    if(!(a._numerador == BigInt(0))){
        if(a._denominador > 0)out << a._numerador << "/" << a._denominador ;
        else out << -a._numerador << "/" << -a._denominador;
    }
