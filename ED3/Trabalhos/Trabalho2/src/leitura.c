...
/*Função responsável por ler o cabeçalho do arquivo indice
    Parametros:
        -> arq: ponteiro para arquivo indice
        -> *f : ponteiro para flag de êxito na leitura
    Retorna: o cabecalho lido na funcao 
*/
indCabecalho le_cab_ind(FILE *arq, int *f){
    indCabecalho cab;

    int n = ftell(arq);                         // guarda endereço do ponteiro de arquivo

    fseek(arq, 0, SEEK_SET);                    // volta para o início do arquivo de indice

    // leitura dos dados do cabecalho
    if( fread(&cab.status, sizeof(char), 1, arq) <= 0 ){
        *f = ERRO;
        return cab;
    }
    ...
    fseek(arq, n, SEEK_SET);                    // ponteiro de arquivo recebe endereco inicial

    *f = 1;                                      // flag de êxito na leitura
    
    return cab;
}

/*Função responsável por ler um nodo do arquivo indice
    Parametros:
        -> arq: ponteiro para arquivo indice
        -> *f : ponteiro para flag de êxito na leitura
    Retorna: o nodo lido na funcao 
*/
indNodo le_nodo_ind(FILE *arq, int *f){
    indNodo nodo;

    // leitura dos informacoes sobre o nodo atual
    if( fread(&nodo.folha, sizeof(char), 1, arq) <= 0 ){
        *f = ERRO;
        return nodo;
    }
    fread(&nodo.nroChavesNo, sizeof(int), 1, arq);
    fread(&nodo.RRNdoNo, sizeof(int), 1, arq);

    // loop de leitura de ponteiros para nos filhos, chaves e referencias do nodo
    for(int i = 0; i < (ORDEM-1); i++){
        fread(&nodo.P[i], sizeof(int), 1, arq);
        fread(&nodo.C[i], sizeof(long), 1, arq);
        fread(&nodo.Pr[i], sizeof(long), 1, arq);
    }
    
    fread(&nodo.P[ORDEM - 1], sizeof(int), 1, arq);     // leitura do ultimo ponteiro do nodo filho
...
/*Função responsável por ler um nodo com dado RRN do arquivo indice
    Parametros:
        -> arq: ponteiro para arquivo indice
        -> *f : ponteiro para flag de êxito na leitura
    Retorna: o nodo lido na funcao 
...
indNodo le_indNodo_RRN(FILE *arq, int *f, int RRN)
{
    ...
    fseek(arq, TAM_INDPAG*(RRN+1), SEEK_SET);   // ponteiro adquire byte-offset do nodo a ser lido           

    nodo = le_nodo_ind(arq, f);                 // leitura do nodo desejado

    return nodo;                                // retorno do nodo lido na posição desejada
}